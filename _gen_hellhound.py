...
Drawn at 4x then downscaled with smoothing. Iconic / stylized so it reads
at the ~28 px size it'll appear at in game.
...
# ---- Style: stylized cartoony, very bold outlines ----
# The dog is FACING +X (east), so head/snout point right.
# Bold colours + outlines so the silhouette stays readable
# at the ~32 px size the game renders it at.
...
def main():
    ...
    # Long curved tail trailing west and up.
    tail_pts = []
    for i in range(0, 22):
        t = i / 21
        ...
        y = int(cy - math.sin(t * math.pi * 0.9) * RENDER * 0.20
                   - t * RENDER * 0.06)
        tail_pts.append((x, y))
    # Outline + base + highlight layers along the tail spline.
    ...
    for sign in (-1, 1):
        lx = int(RENDER * 0.38)
        ly_top = cy + sign * int(RENDER * 0.20)
        ly_bot = cy + sign * int(RENDER * 0.36)
        # Outline
        pygame.draw.line(surf, OUTLINE, (lx, ly_top), (lx, ly_bot), 28)
        pygame.draw.line(surf, FUR_DARK, (lx, ly_top), (lx, ly_bot), 20)
        # Paw
        ellipse_o(surf, lx, ly_bot + sign * 4, 36, 24, FUR_DARK, outline_width=8)

    # -------- Body --------
    body_cx = int(RENDER * 0.50)
    body_w = int(RENDER * 0.50)
    body_h = int(RENDER * 0.42)
    ellipse_o(surf, body_cx, cy, body_w, body_h, FUR, outline_width=14)
    # Spine highlight
    pygame.draw.ellipse(
        surf, FUR_LIGHT,
        pygame.Rect(body_cx - int(body_w * 0.38), cy - int(body_h * 0.30),
                    int(body_w * 0.72), int(body_h * 0.36)),
    )
    ...
    for side in (-1, 1):
        ...
        # inflate by drawing dilated polygon
    ...
    # Mouth (split snout horizontally)
    mouth_y = cy + 6
    ...
    # Teeth — three on top, two on bottom
    for i in range(3):
